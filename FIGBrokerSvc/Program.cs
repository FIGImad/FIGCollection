...
namespace FIGBrokerSvc
{
    public class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            {
                c.ServiceName = ResolveWindowsServiceName(args, "FIGBrokerSvc");
            });
            #endregion WindowsServiceURLS

            #region Configuration
            string settingsFilename = "broker-settings";
            ...
            try
            {
                ...
                builder.Host.UseIBKRProvider(sp =>
                {
                    BrokerRepo.Initialize(sp);
                    var adapters = BrokerRepo.GetBrokerAccounts();
                    // select enabled ones only

                    string serviceId = (sp.GetRequiredService<IConfiguration>()["ControllerConfig:Id"] ?? string.Empty)
                                        .Trim(' ', '\r', '\n', '\t').ToUpper();

                    var adapterList = adapters
                        .Where(a => a.Enable &&
                               string.Equals(a.AccountServiceId.Trim(' ', '\r', '\n', '\t').ToUpper(),
                                             serviceId, StringComparison.Ordinal))
                        .Select(a => a.ToProviderOptions())
                        .ToList();
                    return adapterList;
                });
            }
