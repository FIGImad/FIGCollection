...
namespace FIGBrokerSvc.Services
{
    public class OrderService : IHostedService
    {
        ...
        private async Task<OrderStatusDto?> PlaceOrderBase(OrderRequestDto orderInstruction, CancellationToken cancellationToken, bool waitForResponse = true)
        {
            ...
            try
            {
                _logger.LogInformation("OrderTracking: PlaceOrder received | OrderRefId={OrderRefId}, AccountId={AccountId}, Ticker={Ticker}, OrderType={OrderType}, Qty={Qty}, Price={Price}",
                    orderInstruction.OrderRefId, orderInstruction.AccountId, orderInstruction.Ticker.Symbol, orderInstruction.OrderType, orderInstruction.Qty, orderInstruction.Price);
                ...
                _logger.LogInformation("OrderTracking: Order validated, submitting | OrderRefId={OrderRefId}, AccountId={AccountId}, Ticker={Ticker}, OrderType={OrderType}, Qty={Qty}, Price={Price}",
                    orderInstruction.OrderRefId, brokerAccount.AccountId, ticker.Symbol, orderInstruction.OrderType, orderInstruction.Qty, orderInstruction.Price);
                ...
                _logger.LogInformation("OrderTracking: Order record created in DB | OrderId={OrderId}, OrderRefId={OrderRefId}", oid, orderInstruction.OrderRefId);
                ...
                try
                {
                    ...
                    _logger.LogInformation("OrderTracking: Order submitted to broker | OrderId={OrderId}, OrderRefId={OrderRefId}, StatusCode={StatusCode}, BrokerRef={BrokerRef}",
                        oid, orderInstruction.OrderRefId, defStatus.StatusCode, defStatus.BrokerRef);

                    // 10-Track order asyncroulsy if still in progress
                    if (OrderStatusCodes.IsInProgress(defStatus.StatusCode))
                    {
                        _logger?.LogInformation("OrderTracking: Order still in progress, tracking asynchronously | OrderId={OrderId}, OrderRefId={OrderRefId}, WaitedSeconds={WaitedSeconds}",
                            oid, orderInstruction.OrderRefId, orderExecTimeout);
        ...
        public async Task<OrderStatusDto?> CancelOrder(OrderCancelRequestDto cancelRequest, CancellationToken cancellationToken, int timeoutMsec = -1)
        {
            ...
            try
            {
                _logger.LogInformation("OrderTracking: CancelOrder received | OrderRefId={OrderRefId}, BrokerRef={BrokerRef}, AccountId={AccountId}",
                    cancelRequest.OrderRefId, cancelRequest.BrokerRef, cancelRequest.AccountId);
                OrderRS? order = null;
                ...
                if (order == null)
                {
                    _logger.LogWarning("OrderTracking: CancelOrder | Order not found for OrderRefId={OrderRefId}, BrokerRef={BrokerRef}", cancelRequest.OrderRefId, cancelRequest.BrokerRef);
                    return defStatus;
                }
                ...
                try
                {
                    ...
                    _logger.LogInformation("OrderTracking: Cancel request processed | OrderId={OrderId}, OrderRefId={OrderRefId}, StatusCode={StatusCode}",
                        order.Id, order.OrderRefId, defStatus.StatusCode);
