    ...
    public class OrderTrackerService : BackgroundService, IOrderTrackerService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    await RunTrackingSession(stoppingToken);

                    if (!stoppingToken.IsCancellationRequested)
                    {
                        _logger.LogWarning(
                            "OrderTrackerService tracking session ended unexpectedly; restarting in {RestartDelaySeconds} seconds",
                ...
                {
                    _logger.LogError(ex,
                        "OrderTrackerService tracking session failed; restarting in {RestartDelaySeconds} seconds",
                        RestartDelay.TotalSeconds);
                }
            ...
            _logger.LogInformation("OrderTrackerService stopping");
        }

        private async Task RunTrackingSession(CancellationToken stoppingToken)
        {
            ...
            foreach (var acct in acctList)
            {
                ...
                string accountId = acct.AccountId.Trim().ToUpperInvariant();
                string acctServiceId = acct.AccountServiceId.Trim(' ', '\r', '\n', '\t').ToUpperInvariant();
                if (ServiceId != acctServiceId)
                {
                    _logger.LogInformation("Broker account {AccountId} does not match service ID, skipping", accountId);
                    continue;
                }

                var provider = _serviceProvider.GetKeyedService<IProviderService>(accountId);
                if (provider == null)
                {
                    _logger.LogWarning("No broker provider registered for account {AccountId}", accountId);
                    continue;
                }

                var callbackQueue = _serviceProvider.GetRequiredKeyedService<ICallbackQueue>(accountId);
                trackingTasks.Add(RunSupervisedLoop(
                    accountId,
                    "provider polling",
                    () => RunProviderTrackingLoop(provider, stoppingToken),
                    stoppingToken));
                trackingTasks.Add(RunSupervisedLoop(
                    accountId,
                    "order callback",
                    () => RunOrderCallbackLoop(callbackQueue, stoppingToken),
                    stoppingToken));
            }

            if (trackingTasks.Count == 0)
            {
                _logger.LogWarning(
                    "OrderTrackerService found no enabled broker accounts for service ID {ServiceId}; configuration will be retried",
                    ServiceId);
                return;
            }
        ...
        private async Task RunSupervisedLoop(
            string accountId,
            string loopName,
            Func<Task> runLoop,
            CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        _logger.LogWarning(
                            "OrderTrackerService {LoopName} loop ended unexpectedly for account {AccountId}; restarting",
                            loopName, accountId);
                    }
                }
                catch (OperationCanceledException) when (stoppingToken.IsCancellationRequested)
                ...
                {
                    _logger.LogError(ex,
                        "OrderTrackerService {LoopName} loop failed for account {AccountId}; restarting",
                        loopName, accountId);
                }
        ...
        private async Task RunProviderTrackingLoop(IProviderService provider, CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    _logger.LogInformation("TrackProviderOrders found {ActiveOrderCount} active order(s) for provider {ProviderId}", orderLst.Count, provider.Id);
        ...
        private async Task HandleError(ErrorResponseDto? err)
        {
            ...
            _logger.LogInformation("OrderTracking: HandleError | OrderId={OrderId}, OrderRefId={OrderRefId}, CurrentStatus={CurrentStatus}, BrokerErrorCode={ErrorCode}",
                order.Id, order.OrderRefId, order.OrderStatus, err.ErrorCode);
            ...
            if (OrderStatusCodes.IsInProgress(order.OrderStatus))
            {
                ...
                _logger.LogWarning("OrderTracking: Order failed due to broker error | OrderId={OrderId}, OrderRefId={OrderRefId}, StatusCode={StatusCode}, ErrorCode={ErrorCode}",
                    order.Id, order.OrderRefId, statusCode, err.ErrorCode);
        ...
        private async Task HandleStatusUpdate(OrderStatusUpdate? statusUpdate)
        {
            ...
            _logger.LogInformation("OrderTracking: HandleStatusUpdate | OrderId={OrderId}, OrderRefId={OrderRefId}, CurrentStatus={CurrentStatus}, NewStatus={NewStatus}",
                order.Id, order.OrderRefId, order.OrderStatus, statusUpdate.StatusCode);
            // order is already completed, update status in database
        ...
        {
            _logger.LogInformation("OrderTracking: HandleOpenOrder | BrokerOrderId={BrokerOrderId}, BrokerRef={BrokerRef}, StatusCode={StatusCode}",
                orderStatus?.BrokerOrderId ?? "N/A", orderStatus?.BrokerRef ?? "N/A", orderStatus?.StatusCode.ToString() ?? "N/A");
            await HandleOrderStatusUpdate(orderStatus, false);
        }

        private async Task HandleCompletedOrder(OrderStatusDto? statusUpdate)
        {
            _logger.LogInformation("OrderTracking: HandleCompletedOrder | BrokerOrderId={BrokerOrderId}, BrokerRef={BrokerRef}, StatusCode={StatusCode}",
                statusUpdate?.BrokerOrderId ?? "N/A", statusUpdate?.BrokerRef ?? "N/A", statusUpdate?.StatusCode.ToString() ?? "N/A");
        ...
        private async Task HandleOrderStatusUpdate(OrderStatusDto? orderStatus, bool isCompleted)
        {
            ...
            if (order is null && orderStatus.BrokerRef.Length > 0)
            {
                ...
                order = BrokerRepo.GetOrderByBrokerRef(orderStatus.BrokerRef);
            }
            if (order is null)
            {
            ...
            {
                _logger.LogInformation(
                    "OrderTracking: Updating order status | OrderId={OrderId}, OrderRefId={OrderRefId}, BrokerRef={BrokerRef}, InProgress={InProgress}, FinalState={FinalState}, StatusCode={StatusCode}",
                    order.Id, order.OrderRefId, order.BrokerRef, isInProgress, isUpdateFinalState, orderStatus.StatusCode);
                ...
                BrokerRepo.UpdateOrderStatusSimple(order.Id, orderStatus);
                await _apiATS.SendStatusUpdate(orderStatus);
                if (isUpdateFinalState)
                {
                    _logger.LogInformation("OrderTracking: Order reached final state | OrderId={OrderId}, OrderRefId={OrderRefId}, StatusCode={StatusCode}, FilledQty={FilledQty}, AvgFillPrice={AvgFillPrice}",
                        order.Id, order.OrderRefId, orderStatus.StatusCode, orderStatus.FilledQty, orderStatus.AvgFillPrice);
                }
            }
        }
    }
